...
7. Code-context literal capitalization: when the prose refers to a programming-language literal value (e.g., booleans, null/None, undefined, this/self), the literal MUST match the EXACT casing as it appears in the source code. Do NOT capitalize it as English prose ("becomes True" when the code uses \`true\` is WRONG — must be "becomes \`true\`"). Conversely, do NOT lowercase a literal whose source code uses capitalized form. Examples by language: JS/TS uses lowercase \`true\`/\`false\`/\`null\`/\`undefined\`; Python uses capitalized \`True\`/\`False\`/\`None\`. Always preserve source casing.
