...
describe('assertRequestResolved', () => {
  ...
  test('全件が対象外なら失敗する', () => {
    assert.throws(
      () => assertRequestResolved(['content/old/posts/a.md'], { files: [], dropped: ['content/old/posts/a.md'] }),
      /対象外/,
    );
  });
  ...
  test('1件でも対象があれば通す', () => {
    assert.doesNotThrow(() =>
      assertRequestResolved(['a.md', 'b.json'], { files: ['/repo/content/notion/posts/a.md'], dropped: ['b.json'] }),
    );
  });
