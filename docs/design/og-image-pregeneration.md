...
レンダラ実装を変更した場合は、記事に差分がないため sync 経由では生成されない。引数なしで生成スクリプトを実行して全記事を作り直す。古い画像はR2に残るため、参照が切り替わるまで壊れない。
...
公開判定は生成時点の時刻で行う。公開日が未来の記事は対象外となるが、その日付が到来しても生成は起動しない。生成対象は sync が書き出した記事に限られ、日付の到来では記事に差分が出ないためである。

一方でサイトのビルドは `isPast(created_time)` で判定するため、記事は公開される。結果として予約投稿はOG画像を持たないまま公開される。

段階4でSSRルートを削除すると、この記事のOG画像は404になる。段階4に進む前に、引数なしでの生成を定期実行するか、予約投稿を使わない運用にするかを決める必要がある。
