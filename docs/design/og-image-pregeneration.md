...
呼び出し側が守る規則は次の4つで、いずれも終了コードと報告だけで判断できる。自分が何を渡したか、`--all` だったかを覚えて判断させると、規則がこちらとあちらに二重に置かれ、どちらかを直したときにずれる。
...
- `staged` が真の回だけ、`rendered` と `outputDir` にあるファイルの数を突き合わせる。場所は報告が持つので組み立てない (アップロードに渡すのはその1つ上で、そちらを数えると `og/` 1件になり必ず食い違う)。偽の回は出力先を作り直しておらず、前の回の画像が残りうるため一致しない。
