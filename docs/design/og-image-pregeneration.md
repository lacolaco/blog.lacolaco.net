...
- 数える場所 (`outputDir`) と送る場所 (`stagingDir`) は、どちらも報告が持つ。片方からもう片方を導かない。出力の階層を変えたときに、突き合わせは通るのに送信だけが別の場所を指す。また数えるのは `outputDir` であって送る場所ではない。いまは送る場所を再帰して数えても同数になるが、そこへ別の出力を置いたときに黙って食い違う。
