...
| Cloud Run環境変数 | 全環境 | `GCP_PROJECT_ID`, `FIRESTORE_DATABASE` |
...
全環境変数は明示的に設定必須。デフォルト値なし。未設定時はエラー。

- `GCP_PROJECT_ID`: GCPプロジェクトID
- `FIRESTORE_DATABASE`: Firestoreデータベース名 (`likes-production` or `likes-preview`)
...
- uses: ./.github/actions/deploy-cloudrun
  with:
    env_vars: |
      GCP_PROJECT_ID=blog-lacolaco-net
