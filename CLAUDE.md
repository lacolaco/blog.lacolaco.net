...
2. **変更の影響範囲を設計フェーズで網羅的に列挙せよ**。1つの側面（例: 表示サイズ）だけで設計に入るな。関連する全側面（パフォーマンス、CLS、アクセシビリティ、既存機能との互換性、キャッシュ等）を初期設計で洗い出してから着手する。UIに限らない: 計測・分析基盤なら指標次元（PV/UU/セッション）とプライバシー表記、データ蓄積なら遡及可否・保持期間もこの列挙の対象
